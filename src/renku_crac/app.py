...
class Server:
    ...
    def _get_resource_pool(self):
        async def handler(request: Request, resource_pool_id: int):
            res = await self.config.rp_repo.get_resource_pools(resource_pool_id, name=request.args.get("name"))
            if len(res) < 1:
    ...
    def _get_resource_pools(self):
        async def handler(_: Request):
            res = await self.config.rp_repo.get_resource_pools()
    ...
    def _post_resource_pools(self):
        ...
        async def handler(_: Request, body: apispec.ResourcePool):
            ...
            res = await self.config.rp_repo.insert_resource_pool(rp)
    ...
    def _post_user(self):
        ...
        async def handler(_: Request, body: apispec.UserWithId):
            users = await self.config.user_repo.get_users(keycloak_id=body.id)
            if len(users) == 1:
                ...
                if user.username != body.username:
                    ...
                    user = await self.config.user_repo.update_user(
                        keycloak_id=cast(int, user.id), username=body.username
                    )
                return json(
            ...
            user = await self.config.user_repo.insert_user(user_kc)
            return json(
    ...
    def _get_users(self):
        async def handler(request: Request):
            res = await self.config.user_repo.get_users(username=request.args.get("username"))
            return json(
                [
    ...
    def _delete_user(self):
        async def handler(_: Request, id: str):
            await self.config.user_repo.delete_user(id)
    ...
    def _get_resource_pool_users(self):
        async def handler(request: Request, resource_pool_id: int):
            res = await self.config.user_repo.get_users(
    ...
    def _get_resource_pool_user(self):
        async def handler(_: Request, resource_pool_id: int, user_id: str):
            res = await self.config.user_repo.get_users(keycloak_id=user_id, resource_pool_id=resource_pool_id)
            if len(res) < 1:
                raise errors.MissingResourceError(
    ...
    def _get_resource_pool_classes(self):
        async def handler(request: Request, resource_pool_id: int):
            res = await self.config.rp_repo.get_classes(
                resource_pool_id=resource_pool_id, name=request.args.get("name")
            )
    ...
    def _get_resource_pool_class(self):
        async def handler(_: Request, resource_pool_id: int, class_id: int):
            res = await self.config.rp_repo.get_classes(resource_pool_id=resource_pool_id, id=class_id)
            if len(res) < 1:
                raise errors.MissingResourceError(
    ...
    def _delete_resource_pool_class(self):
        async def handler(_: Request, resource_pool_id: int, class_id: int):
            await self.config.rp_repo.delete_resource_class(
                resource_pool_id=resource_pool_id, resource_class_id=class_id
            )
            return HTTPResponse(status=204)
    ...
    def _put_patch_resource_pool_class(self, validation: Type[apispec.ResourceClassPatch | apispec.ResourceClass]):
        ...
        async def handler(
            ...
            cls = await self.config.rp_repo.update_resource_class(
    ...
    def _post_resource_pool_class(self):
        ...
        async def handler(_: Request, body: apispec.ResourceClass, resource_pool_id: int):
            rps = await self.config.rp_repo.get_resource_pools(id=resource_pool_id)
            if len(rps) < 1:
                raise errors.MissingResourceError(
                    message=f"The resource pool with id {resource_pool_id} cannot be found."
                )
            ...
            res = await self.config.rp_repo.insert_resource_class(cls, resource_pool_id=rp.id)
    ...
    def _get_quota(self):
        async def handler(_: Request, resource_pool_id: int):
            res = await self.config.rp_repo.get_quota(resource_pool_id=resource_pool_id)
            if res is None:
                raise errors.MissingResourceError(
    ...
    def _put_patch_quota(self, validation=Type[apispec.ResourcesPatch | apispec.Resources]):
        ...
        async def handler(_: Request, resource_pool_id: int, body: apispec.ResourcesPatch | apispec.Resources):
            res = await self.config.rp_repo.update_quota(
                resource_pool_id=resource_pool_id, **body.dict(exclude_none=True)
            )
            if res is None:
                raise errors.MissingResourceError(
    ...
    def _put_patch_resource_pool(self, validation=Type[apispec.ResourcePoolPut | apispec.ResourcePoolPatch]):
        ...
        async def handler(_: Request, resource_pool_id: int, body: apispec.ResourcePoolPut | apispec.ResourcePoolPatch):
            res = await self.config.rp_repo.update_resource_pool(id=resource_pool_id, **body.dict(exclude_none=True))
            if res is None:
                raise errors.MissingResourceError(
                    message=f"The resource pool with ID {resource_pool_id} cannot be found."
                )
    ...
    def _delete_resource_pool(self):
        async def handler(_: Request, resource_pool_id: int):
            await self.config.rp_repo.delete_resource_pool(id=resource_pool_id)
            return HTTPResponse(status=204)
    ...
    def _get_user_resource_pools(self):
        async def handler(_: Request, user_id: str):
            rps = await self.config.user_repo.get_user_resource_pools(keycloak_id=user_id)
    ...
    def _post_put_user_resource_pools(self, *, post: bool = True):
        async def handler(request: Request, user_id: str):
            ...
            rps = await self.config.user_repo.update_user_resource_pools(
    ...
    def _post_put_resource_pool_users(self, *, post: bool = True):
        async def handler(request: Request, resource_pool_id: int):
            ...
            rp = await self.config.rp_repo.update_resource_pool_users(
    ...
    def _delete_resource_pool_user(self):
        async def handler(request: Request, resource_pool_id: int, user_id: str):
            await self.config.rp_repo.delete_resource_pool_user(resource_pool_id=resource_pool_id, keycloak_id=user_id)
            return HTTPResponse(status=204)
