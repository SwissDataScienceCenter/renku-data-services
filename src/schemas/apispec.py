...
#   timestamp: 2023-05-03T16:08:12+00:00
...
class ResourcePoolPatch(BaseAPISpec):
    ...
    classes: Optional[List[ResourceClassPatchWithId]] = Field(
        None,
        example=[{"name": "resource class 1", "id": 1}, {"cpu": 4.5, "storage": 10000, "id": 2}],
        min_items=1,
        unique_items=True,
    )
    name: Optional[str] = Field(
...
class ResourcePoolPut(BaseAPISpec):
    ...
    classes: List[ResourceClassesWithIdItem] = Field(
        ...,
        example=[
            {"name": "resource class 1", "cpu": 1.5, "memory": 2, "gpu": 0, "storage": 100, "id": 1},
            {"name": "resource class 2", "cpu": 4.5, "memory": 10, "gpu": 2, "storage": 10000, "id": 2},
        ],
        unique_items=True,
    )
    name: str = Field(..., description="A name for a specific resource", example="the name of a resource", min_length=5)
