...
async def sanic_client(secret_storage_app_config: Config, users: List[UserInfo]) -> SanicASGITestClient:
    secret_storage_app_config.kc_api = DummyKeycloakAPI(users=get_kc_users(users))
    app = Sanic(secret_storage_app_config.app_name)
    app = register_all_handlers(app, secret_storage_app_config)
