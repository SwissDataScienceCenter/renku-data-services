...
async def test_group_slug_change_updates_project_search(
    ...
    slug = "test-group"
    group = await create_group_model("Test Group", user=regular_user, slug=slug)
    project = await create_project_model("Project", user=regular_user, visibility="public", namespace=group.slug)
    ...
    resources = await search_query(f"namespace:{group.slug} type:dataconnector,project", user=regular_user)
    ...
    resources = await search_query(f"namespace:{new_slug} type:dataconnector,project", user=regular_user)
    assert_search_result(resources, [data_connector, project])

    # Verify resources aren't found in the old namespace anymore
    resources = await search_query(f"namespace:{slug} type:dataconnector,project", user=regular_user)
