...
async def bootstrap_admins(
    sanic_client_with_migrations, app_config_instance: DependencyManager, event_loop, admin_user: UserInfo
) -> None:
...
@pytest_asyncio.fixture(scope="session")
async def sanic_app_no_migrations(app_config: DependencyManager, users: list[UserInfo], admin_user: UserInfo) -> Sanic:
...
@pytest_asyncio.fixture
async def search_reprovision(app_config_instance: DependencyManager, search_push_updates):
...
@pytest_asyncio.fixture
async def search_push_updates(app_config_instance: DependencyManager):
