...
def _convert_unsaved_storage_url(storage: apispec.CloudStorageUrlV2) -> models.CloudStorageCore:
    ...
    config, source_path = parse_storage_url(storage.storage_url)
    ...
    match body.storage:
        ...
        case apispec.CloudStorageCorePost() if body.storage.storage_type == "doi":
            ...
            if doi_metadata is None:
                raise errors.ValidationError(message=f"Cannot get metadata for the data connector with doi {doi}")
...
async def prevalidate_unsaved_global_data_connector(
    ...
    return models.PrevalidatedGlobalDataConnector(
        data_connector=models.UnsavedGlobalDataConnector(
            name=doi_metadata.dataset.name,
            slug=slug,
            visibility=Visibility.PUBLIC,
            created_by="",
            storage=storage,
            description=doi_metadata.dataset.description,
            keywords=doi_metadata.dataset.keywords,
            doi=doi,
...
async def convert_scicat_v1_data_connector_to_s3(
    ...
    scicat_url = create_scicat_metadata_url(doi)
    headers = {"accept": "application/ld+json"}

    clnt = httpx.AsyncClient(follow_redirects=True, timeout=5)
    async with clnt:
        res = await clnt.get(scicat_url, headers=headers)
