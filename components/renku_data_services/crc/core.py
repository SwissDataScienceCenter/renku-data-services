...
def validate_resource_pool_put_or_patch(
    ...
    return models.ResourcePoolPatch(
        ...
        cluster_id=ULID.from_str(body.cluster_id) if body.cluster_id else None,
        platform=platform,
        cpu_limit_factor=RESET
        if body.cpu_limit_factor is None or body.cpu_limit_factor == 0
        else body.cpu_limit_factor,
    )
...
def validate_resource_pool_update(existing: models.ResourcePool, update: models.ResourcePoolPatch) -> None:
    ...
    match (existing.remote, update.remote):
        case (_, ResetType.Reset):
            remote = RESET
        case (None, models.RemoteConfigurationFirecrestPatch()):
        ...
        case (None, models.RemoteConfigurationRunaiPatch()):
        ...
        case _:
            raise errors.ValidationError(
                ...
                f"Current state: '{existing.remote}', patch: '{update.remote}'",
            )
...
def validate_firecrest_api_url(url: str | None) -> None:
    """Validate the URL to the FirecREST API."""
    if url is None:
        raise errors.ValidationError(
            message="The firecrest api url must be provided.",
            quiet=True,
        )
    parsed = urlparse(url)
...
def validate_runai_base_url(url: str | None) -> None:
    """Validate the URL to the Run:AI API."""
    if url is None:
        raise errors.ValidationError(
            message="The runai base url must be provided.",
            quiet=True,
        )
    parsed = urlparse(url)
    if not parsed.netloc:
