...
class ClustersBP(CustomBlueprint):
    ...
    def post(self) -> BlueprintFactoryResponse:
        ...
        async def _handler(_request: Request, _user: base_models.APIUser, body: apispec.Cluster) -> HTTPResponse:
            ...
            cluster = await self.repo.insert(cluster)
    ...
    def put(self) -> BlueprintFactoryResponse:
        ...
        async def _handler(
            ...
            cluster = SavedCluster(name=body.name, id=cluster_id)
            cluster = await self.repo.update(cluster, cluster_id)
