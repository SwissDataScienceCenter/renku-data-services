...
def notebooks_internal_gitlab_authenticate(
    ...
    [Callable[Concatenate[Request, AuthenticatedAPIUser | AnonymousAPIUser, APIUser, _P], Awaitable[_T]]],
    ...
    def decorator(
        f: Callable[Concatenate[Request, AuthenticatedAPIUser | AnonymousAPIUser, APIUser, _P], Awaitable[_T]],
    ) -> Callable[Concatenate[Request, AuthenticatedAPIUser | AnonymousAPIUser, APIUser, _P], Coroutine[Any, Any, _T]]:
        @wraps(f)
        async def decorated_function(
            ...
            **kwargs: _P.kwargs,
            ...
            gl_user = await authenticator.authenticate(access_token, request)
            return await f(request, user, gl_user, *args, **kwargs)
