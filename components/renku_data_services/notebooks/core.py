...
def notebooks_info(config: _NotebooksConfig) -> dict:
...
async def user_servers(
    config: _NotebooksConfig, user: AnonymousAPIUser | AuthenticatedAPIUser, filter_attrs: list[dict]
) -> dict:
...
async def user_server(
    config: _NotebooksConfig, user: AnonymousAPIUser | AuthenticatedAPIUser, server_name: str
) -> UserServerManifest:
...
def server_options(config: _NotebooksConfig) -> dict:
...
async def server_logs(
    config: _NotebooksConfig, user: AnonymousAPIUser | AuthenticatedAPIUser, server_name: str, max_lines: int
) -> dict:
...
def docker_image_exists(config: _NotebooksConfig, image_url: str, internal_gitlab_user: APIUser) -> bool:
