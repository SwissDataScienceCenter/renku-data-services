...
#   filename:  <stdin>
#   timestamp: 2024-07-13T15:44:03+00:00
...
class DataSource(BaseCRD):
    ...
    type: Type2 = Field("rclone", description="The data source type")
...
class HttpGet(BaseCRD):
    ...
    scheme: Optional[str] = Field(
        None, description="Scheme to use for connecting to the host.\nDefaults to HTTP."
    )
...
class Quobyte(BaseCRD):
    ...
    group: Optional[str] = Field(
        None, description="group to map volume access to\nDefault is no group"
    )
    readOnly: Optional[bool] = Field(
        None,
...
class Storage(BaseCRD):
    ...
    mountPath: str = Field(
        "/workspace", description="The absolute mount path for the session volume"
    )
...
class Session(BaseCRD):
    ...
    extraVolumeMounts: Optional[List[ExtraVolumeMount]] = Field(
        None, description="Additional volume mounts for the session container"
    )
    image: str
    port: int = Field(
        ...,
        description="The TCP port on the session Pod where the session can be accessed. Can point to either the\nsession container or any additional container that is added. K8s port names are not accepted.",
        gt=0,
    )
    ...
    runAsGroup: int = Field(1000, ge=0)
    runAsUser: int = Field(1000, ge=0)
    storage: Storage = {}
    urlPath: str = Field(
        "/",
        description="The path where the session can be accessed. If an ingress is specified, this value must\nbe a subpath of the ingress `pathPrefix` field.",
    )
    workingDir: Optional[str] = Field(
        None,
...
class Spec(BaseCRD):
    ...
    authentication: Optional[Authentication] = Field(
        None, description="Authentication configuration for the session"
    )
    ...
    culling: Optional[Culling] = Field(None, description="Culling configuration")
    dataSources: Optional[List[DataSource]] = Field(
        None, description="A list of data sources that should be added to the session"
    )
