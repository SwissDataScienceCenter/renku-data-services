...
#   timestamp: 2026-09-16T09:47:31+00:00
...
class DataConnectorAccessPolicyName(StrEnum):
    disabled = "disabled"
    readOnly = "readOnly"
    readWrite = "readWrite"


class DataConnectorAccessPolicy(BaseAPISpec):
    policy: DataConnectorAccessPolicyName


class RepositoryAccessPolicyName(StrEnum):
    disabled = "disabled"
    readOnly = "readOnly"
    readWrite = "readWrite"


class ProjectRepositorySurrogateKey(BaseAPISpec):
    repository_id: int = Field(..., ge=0)


class GitReference(RootModel[str]):
    root: str = Field(
        ...,
        description="A git references",
        examples=["refs/heads/main"],
        pattern="^refs/(heads|tags)/.*$",
    )


class RepositoryAccessPolicy(BaseAPISpec):
    policy: RepositoryAccessPolicyName
    writable_references: list[GitReference] = Field(
        ..., description="List of git references"
    )


class SessionLauncherRepositoryPatch(
    ProjectRepositorySurrogateKey, RepositoryAccessPolicy
):
    pass


class SecretAccessPolicyName(StrEnum):
    disabled = "disabled"
    enabled = "enabled"


class SecretAccessPolicy(BaseAPISpec):
...
class SessionLauncherRepositoryListPatch(RootModel[Any]):
    root: Any
...
class DataConnectorKey(BaseAPISpec):
...
class SessionLauncherDataConnector(DataConnectorKey, DataConnectorAccessPolicy):
    pass


class RepositoryNaturalKey(BaseAPISpec):
    url: str = Field(..., description="A git repository URL")


class RepositoryCompositeKey(ProjectRepositorySurrogateKey, RepositoryNaturalKey):
    pass


class SessionLauncherRepositoryPatchList(
    RootModel[list[SessionLauncherRepositoryPatch]]
):
    root: list[SessionLauncherRepositoryPatch] = Field(
        ...,
        description="Patch a list of git repository properties of a Renku 2.0 session\n",
    )


class SessionLauncherRepository(RepositoryCompositeKey, RepositoryAccessPolicy):
    pass


class SecretSlotKey(BaseAPISpec):
...
class SessionLauncherSecret(SecretSlotKey, SecretAccessPolicy):
    pass


class SessionLauncherSecretPatchList(RootModel[list[SessionLauncherSecret]]):
    root: list[SessionLauncherSecret] = Field(
        ..., description="A list of session launcher secret patches"
    )
...
class SessionLauncherDataConnectorList(RootModel[list[SessionLauncherDataConnector]]):
    root: list[SessionLauncherDataConnector] = Field(
        ..., description="A list of data connector properties\n"
    )


class SessionLauncherDataConnectorPatchList(
    RootModel[list[SessionLauncherDataConnector]]
):
    root: list[SessionLauncherDataConnector] = Field(
        ..., description="A list of data connector property patches\n"
    )


class SessionLauncherRepositoryList(RootModel[list[SessionLauncherRepository]]):
    root: list[SessionLauncherRepository] = Field(
        ...,
        description="A list of repository access policy paremeters of a Renku 2.0 session\n",
    )


class SessionLauncherSecretList(RootModel[list[SessionLauncherSecret]]):
    root: list[SessionLauncherSecret] = Field(
