...
#   timestamp: 2026-09-25T17:35:01+00:00
...
class ProjectRepositoryId(BaseAPISpec):
    repository_id: int = Field(..., ge=0)


class GitReference(RootModel[str]):
    root: str = Field(..., description="A git branch", examples=["main"], min_length=1)


class RepositoryAccessPolicy(BaseAPISpec):
    policy: RepositoryAccessPolicyName
    writable_references: list[GitReference] | None = Field(
        None, description="List of git references"
    )


class SessionLauncherRepositoryPatch(ProjectRepositoryId, RepositoryAccessPolicy):
    pass
...
class DataConnectorKey(BaseAPISpec):
    data_connector_link_id: str = Field(
        ...,
        description="ULID identifier",
        max_length=26,
        min_length=26,
...
class RepositoryUrl(BaseAPISpec):
    url: str = Field(..., description="A git repository URL")


class RepositoryCompositeKey(ProjectRepositoryId, RepositoryUrl):
    pass
