...
class SessionRepository:
    ...
    async def insert_environment(
        ...
        environment = schemas.EnvironmentORM.load(new_environment)
    ...
    async def get_project_launchers(self, user: base_models.APIUser, project_id: ULID) -> list[models.SessionLauncher]:
        ...
        async with self.session_maker() as session:
            res = await session.scalars(
                select(schemas.SessionLauncherORM)
                .where(schemas.SessionLauncherORM.project_id == str(project_id))
                .order_by(schemas.SessionLauncherORM.creation_date.desc())
            )
            launcher = res.all()
            return [item.dump() for item in launcher]
    ...
    async def insert_launcher(
        ...
        async with self.session_maker() as session, session.begin():
            res = await session.scalars(select(schemas.ProjectORM).where(schemas.ProjectORM.id == str(project_id)))
            ...
            launcher = schemas.SessionLauncherORM.load(new_launcher)
    ...
    async def update_launcher(
        ...
        async with self.session_maker() as session, session.begin():
            ...
            if launcher.environment_kind == models.EnvironmentKind.global_environment:
                launcher.container_image = None
            if launcher.environment_kind == models.EnvironmentKind.container_image:
