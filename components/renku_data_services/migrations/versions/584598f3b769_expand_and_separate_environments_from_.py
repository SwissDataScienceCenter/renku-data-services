...
def downgrade() -> None:
    ...
    op.execute(
        "UPDATE sessions.launchers "
        "SET sessions.launchers.default_url = sessions.environments.default_url, "
        "sessions.launchers.container_image = sessions.environments.container_image "
        "sessions.laucnhers.environment_kind = 'container_image' "
        "FROM sessions.environments "
        "WHERE sessions.launchers.environment_id = sessions.environments.id AND "
        "sessions.environments.environment_kind = 'CUSTOM'"
    )
    op.execute(
        "UPDATE sessions.launchers "
        "SET sessions.launchers.environment_kind = 'global_environment' "
        "WHERE environment_kind IS NULL"
    )
