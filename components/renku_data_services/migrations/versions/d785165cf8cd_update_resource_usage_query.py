...
def upgrade() -> None:
    ...
    with
      cpu_obs as (
        select id, capture_date, lead(capture_date) over (order by capture_date asc) as cpu_next_date
        from resource_pools.resource_requests_log
        where cpu_request is not null and user_id is not null
      ),
      mem_obs as (
        select id, capture_date, lead(capture_date) over (order by capture_date asc) as mem_next_date
        from resource_pools.resource_requests_log
        where memory_request is not null and user_id is not null
      ),
      disk_obs as (
        select id, capture_date, lead(capture_date) over (order by capture_date asc) as disk_next_date
        from resource_pools.resource_requests_log
        where disk_request is not null and user_id is not null
      ),
      gpu_obs as (
        select id, capture_date, lead(capture_date) over (order by capture_date asc) as gpu_next_date
        from resource_pools.resource_requests_log
