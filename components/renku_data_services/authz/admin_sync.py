...
def sync_admins_from_keycloak(kc_api: IKeycloakAPI, authz: Authz):
    ...
    for admin_id in kc_admin_user_ids:
        ...
        authz.client.WriteRelationships(change.apply)
    authz_admins = authz._get_admin_user_ids()
    for admin_id in authz_admins:
        if admin_id not in kc_admin_user_ids:
            change = authz._remove_admin(admin_id)
            authz.client.WriteRelationships(change.apply)
