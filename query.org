...
select count(*) from common.resource_requests_log
#+end_src
  ...
  with
    ...
    requests_diffs as (
      select
        ...
        disk_request,
        capture_date,
        lead(capture_date) over cpu_w as next_cpu_date ,
        lag(capture_date) over cpu_w as prev_cpu_date ,
    ...
    request_hours as (
      select *,
        cpu_request * (extract(epoch from cpu_time_diff) / 3600) as cpu_hours,
        memory_request * (extract(epoch from mem_time_diff) / 3600) as mem_hours,
        disk_request * (extract(epoch from disk_time_diff) / 3600) as disk_hours
      from final_requests_diffs
    ),
    per_user as (
      select
        user_id,
        sum(cpu_hours) as cpu_hours,
        sum(mem_hours) as mem_hours,
        sum(disk_hours) as disk_hours
      from request_hours
      group by user_id
    )
...
| SET                                  |                     |                    |                    |
|--------------------------------------+---------------------+--------------------+--------------------|
| user_id                              |           cpu_hours |          mem_hours |         disk_hours |
| 1a6d4c40-e500-419d-9812-22d6d1c27aa0 |                   0 |                  0 |  4122572080.924443 |
| 4cf16e99-62fe-4889-90ba-38b2d02bd293 |                   0 |                  0 |  4122572080.924443 |
| 5daa2891-5310-4428-895d-10f859613223 |                   0 |                  0 |  4122572080.924443 |
| c59e6907-e670-4e03-a1b7-18616bb4678e |  1.7129999999999996 | 3512589789.8666677 | 3585402907.3066654 |
| e25cd000-59be-4e52-86aa-fb4fc60a12c8 | 0.17333333333333334 |  363506346.6666666 | 178956970.66666666 |
| f5f1ebe7-6946-4c2c-b4e5-18ac2d4f41e4 |                   0 |                  0 |  4122572080.924443 |

This could serve as the base for doing time-bucket calculations, like
usage per hour, per week etc.

Problem: edges, from one hour to the next, or one day to the next.
